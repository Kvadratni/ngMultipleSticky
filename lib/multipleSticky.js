(function () {
	...
	module.directive('sticky', function() {
		...
		function linkFn($scope, $elem, $attrs) {
			var mediaQuery, stickyClass, bodyClass,compactOnCount,compactClass, elem, $window, $body,
			...
			function checkIfShouldStick() {
                ...
                {
                	if(compactOnCount!=-1){
	                	if ( anchor === 'top' && $scope.Stickying.Top.length>=compactOnCount )
	                		$elem.addClass(compactClass);
	                	else if($scope.Stickying.Bottom.length>=compactOnCount)
	                		$elem.addClass(compactClass);	
	                	else
