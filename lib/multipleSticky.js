	...
	module.directive('sticky', function() {
		return {
			...
			controller : function($scope){
              $scope.Stickying= {Top:[],Bottom:[]};
              this.config = function (){ 
                   return $scope.Stickying;
              }
         },
			link: linkFn
		};
		...
		function linkFn($scope, $elem, $attrs) {
			var mediaQuery, stickyClass, bodyClass, compactOnCount, compactClass, compactOffset, ensureOffset, elem, $window, $body,
			...
			compactClass = $attrs.compactClass || '';

			usePlaceholder = $attrs.useplaceholder == undefined ? false : true;

			initialStyle = $elem.attr('style');

			compactOnCount = typeof $attrs.compactOnCount === 'string' ? 
				parseInt($attrs.compactOnCount) : 
				-1;

				compactOffset = typeof $attrs.compactOffset === 'string' ? 
				parseInt($attrs.compactOffset.replace(/px;?/, '')) : 
				0;
            ...
            ensureOffset = typeof $attrs.ensureOffset === 'string' ?
                $attrs.ensureOffset.toLowerCase().trim() : 'false';

            ensureOffset = (ensureOffset === 'true');
			...
			function checkIfShouldStick() {
                ...
                {
                	if(compactOnCount!=-1 || compactOffset!=0){
	                	if ( anchor === 'top' && (($scope.Stickying.Top.length>=compactOnCount && compactOnCount!=-1) ||scrollTop>=compactOffset))
	                		$elem.addClass(compactClass);
	                	else if(($scope.Stickying.Bottom.length>=compactOnCount && compactOnCount!=-1)||scrollBottom>=compactOffset)
	                		$elem.addClass(compactClass);	
	                	else
	                		$elem.removeClass(compactClass);                	
                	}
                	if(ensureOffset)
            ...
            function calcOffset() {
					...
					var offsetValue=0;
					if ( anchor === 'bottom' ){
						var arr= $scope.Stickying.Bottom;
						var offsetParam = 'bottom';						
					}else{
						var arr = $scope.Stickying.Top;
						var offsetParam = 'top';
					}
					var index = arr.indexOf($elem)!=-1?arr.indexOf($elem):arr.length;
					for (i;i<index;i++){
						var localOffset = (parseInt(arr[i].css(offsetParam).replace(/px;?/, ''))||0)- offsetValue;
						offsetValue += arr[i][0].offsetHeight+localOffset;
					}
					return offsetValue;
            }
			...
			function stickElement() {
				...
				if ( anchor === 'bottom' ) {
					...
					$scope.Stickying.Bottom.push($elem);
				}
				else
				{
					$scope.Stickying.Top.push($elem);
				}
			...
			function unstickElement(fromDirection, scrollTop) {
                ...
                if ( anchor === 'bottom' ) {
					...
					$scope.Stickying.Bottom.splice($elem);
				}
				else
				{
					$scope.Stickying.Top.splice($elem);
				}
